...
namespace MakC.Common
{
    public class LogHelper : ILoggerHelper
    {
        ...
        private string repositoryName = ConfigHelper.Configuration["Logging:Log4Net:Name"];
        /// <summary>
        /// 获取记录器
        /// </summary>
        ...
        private ILog GetLogger(Type source)
        {
            ...
            if(!Loggers.TryGetValue(source, out logger))
            {
                lock (Loggers)
                {
                    if (!Loggers.TryGetValue(source, out logger))
                    {
                        logger = LogManager.GetLogger(repositoryName, source);
